...
import { ClothingSwapInput } from './ClothingSwapInput';
import { LoadingSpinner } from './LoadingSpinner';
import { ImageFile, performClothingSwap } from '../services/geminiService';
...
const CLOTHING_SWAP_LOADING_MESSAGES = [
    'Warming up the virtual sewing machine...',
    'Tailoring the digital fabric...',
    'Getting the perfect fit...',
    'Checking the seams...',
];

export const ClothingSwap: React.FC<ClothingSwapProps> = ({ onBack, onCreationComplete, addToast, initialImage, onClearInitialImage }) => {
    ...
    useEffect(() => {
        ...
        if (isLoading) {
            setLoadingMessage(CLOTHING_SWAP_LOADING_MESSAGES[0]);
            interval = setInterval(() => {
                setLoadingMessage(prev => {
                    const currentIndex = CLOTHING_SWAP_LOADING_MESSAGES.indexOf(prev);
                    const nextIndex = (currentIndex + 1) % CLOTHING_SWAP_LOADING_MESSAGES.length;
                    return CLOTHING_SWAP_LOADING_MESSAGES[nextIndex];
                });
            }, 2500);
        }
    ...
    const handleClothingSwapGenerate = async (personImage: ImageFile, clothingImage: ImageFile) => {
        setIsLoading(true);
        setError(null);
        setEditHistory([]);
        setEditHistoryIndex(-1);
        setIsSaved(false);
        const originalImageUrl = `data:${personImage.type};base64,${personImage.data}`;
        setOriginalImage(originalImageUrl);

        try {
            const imageUrl = await performClothingSwap(personImage, clothingImage);
    ...
    const handleSave = () => {
        if (result && originalImage && !isSaved) {
            onCreationComplete({ type: 'Clothing Swap (BETA)', originalUrl: originalImage, imageUrl: result });
            setIsSaved(true);
        } else if (isSaved) {
    ...
    const renderContent = () => {
        ...
        if (result && originalImage) {
            return (
                <div className="text-center w-full max-w-4xl mx-auto fade-in">
                    ...
                    <h2 className="text-3xl font-bold mb-6 text-white">Clothing Swap (BETA) Result</h2>
        ...
        return <ClothingSwapInput isLoading={isLoading} onGenerate={handleClothingSwapGenerate} onStartOver={handleStartOver} initialImage={initialImageFile}/>;
    };
